...
def generate_exercise(property_adjective=None, adjectivs=None, count=5):
    ...
    if property_adjective == "komparativ_und_superlativ":
        ...
        selected_cases = random.choices(cases_choice, k=count)
        for adj, decl, case, in list(zip(adjectivs, selected_declensions, selected_cases)):
            gender_name, article = list(genders[case].items())[0]
            print('PPPPPPP', decl,case,gender_name, article)
            print('aDJ', adj.declensions)
...
def adjectivs_results_repeat(request):
    ...
    if request.method == "POST":
        ...
        property_adjective = str(request.POST.get('property_adjective'))
        lection_ids = request.POST.getlist('lections')
        user = request.user
        params = {'property_adjective': property_adjective}
        for l_id in lection_ids:
            params.setdefault('lections', []).append(l_id)

        url = reverse("api:exercises:exercises_adjectivs") + "?" + urlencode(params, doseq=True)
        return redirect(url)
